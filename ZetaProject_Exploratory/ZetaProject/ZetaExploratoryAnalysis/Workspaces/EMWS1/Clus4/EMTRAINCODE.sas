...
TRAIN tech=FORGY
;
SAVE outstat=WORK.Clus4_OUTSTAT outmean=EMWS1.Clus4_OUTMEAN;
code file="\\uisnutvdiprof1a\redirected$\bkris2\Documents\ZetaProject\ZetaExploratoryAnalysis\Workspaces\EMWS1\Clus4\DMVQSCORECODE.sas"
group=Clus4
;
run;
quit;
...
*------------------------------------------------------------*;
proc cluster data=EMWS1.Clus4_OUTMEAN method=AVERAGE pseudo outtree=EMWS1.Clus4_CLUSSEED
;
var %dmvqvars;
copy _SEGMENT_;
run;
quit;
proc sort data =EMWS1.Clus4_CLUSSEED out=WORK._SEED_;
by _ncl_;
where (_ccc_ > -99999.0);
run;
data WORK._SEED2_;
