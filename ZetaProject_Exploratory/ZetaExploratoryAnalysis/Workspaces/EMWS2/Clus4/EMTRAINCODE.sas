*------------------------------------------------------------*;
* Clus4: Preliminary Clustering;
*------------------------------------------------------------*;
*------------------------------------------------------------* ;
* Clus4: DMDBClass Macro ;
*------------------------------------------------------------* ;
%macro DMDBClass;
    AnyHealthcare(ASC) CholCheck(ASC) DiffWalk(ASC) Education(ASC) GenHlth(ASC)
   HeartDiseaseorAttack(ASC) HighBP(ASC) HighChol(ASC) HvyAlcoholConsump(ASC)
   Income(ASC) MentHlth(ASC) NoDocbcCost(ASC) PhysHlth(ASC)
%mend DMDBClass;
*------------------------------------------------------------* ;
* Clus4: DMDBVar Macro ;
*------------------------------------------------------------* ;
...
* Clus4: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=EMWS2.Smpl_DATA
...
*------------------------------------------------------------* ;
%macro DMVQNOMINAL;
    AnyHealthcare CholCheck DiffWalk HeartDiseaseorAttack HighBP HighChol
   HvyAlcoholConsump NoDocbcCost
%mend DMVQNOMINAL;
*------------------------------------------------------------* ;
* Clus4: Ordinal Inputs Macro ;
*------------------------------------------------------------* ;
%macro DMVQORDINAL;
...
MAKE outvar=EMWS2.Clus4_OUTVAR;
INITIAL radius=0
;
TRAIN tech=FORGY
;
SAVE outstat=WORK.Clus4_OUTSTAT outmean=EMWS2.Clus4_OUTMEAN;
code file="\\uisnutvdiprof1a\redirected$\bkris2\Documents\ZetaProject\ZetaExploratoryAnalysis\Workspaces\EMWS2\Clus4\DMVQSCORECODE.sas"
group=Clus4
;
run;
quit;
...
%macro dmvqvars;
   ...
   DiffWalk1 HeartDiseaseorAttack0 HeartDiseaseorAttack1 HighBP0 HighBP1 HighChol0
   HighChol1 HvyAlcoholConsump0 HvyAlcoholConsump1 NoDocbcCost0 NoDocbcCost1
   T_Education T_GenHlth T_Income T_MentHlth T_PhysHlth
%mend ;
*------------------------------------------------------------*;
* Clus4: Determining the number of clusters;
*------------------------------------------------------------*;
proc cluster data=EMWS2.Clus4_OUTMEAN method=WARD pseudo outtree=EMWS2.Clus4_CLUSSEED
;
var %dmvqvars;
copy _SEGMENT_;
run;
quit;
proc sort data =EMWS2.Clus4_CLUSSEED out=WORK._SEED_;
by _ncl_;
where (_ccc_ > -99999.0);
run;
data WORK._SEED2_;
...
TRAIN tech=FORGY
;
SAVE outstat=EMWS2.Clus4_OUTSTAT outmean=EMWS2.Clus4_OUTMEAN;
code file="\\uisnutvdiprof1a\redirected$\bkris2\Documents\ZetaProject\ZetaExploratoryAnalysis\Workspaces\EMWS2\Clus4\DMVQSCORECODE.sas"
group=Clus4
;
run;
