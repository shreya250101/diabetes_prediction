...
%let EM_NUM_VARS = 16;
   %macro EM_VARS;
    Age AnyHealthcare BMI CholCheck DiffWalk Education GenHlth
   HeartDiseaseorAttack HighBP HighChol HvyAlcoholConsump Income MentHlth
   NoDocbcCost PhysHlth _SEGMENT_
   %mend EM_VARS;
%let EM_NUM_CLASS = 14;
   %macro EM_CLASS;
    AnyHealthcare CholCheck DiffWalk Education GenHlth HeartDiseaseorAttack HighBP
   HighChol HvyAlcoholConsump Income MentHlth NoDocbcCost PhysHlth _SEGMENT_
   %mend EM_CLASS;
...
%let EM_NUM_REJECTS = 9;
   %macro EM_REJECTS;
    Diabetes_012 Distance Fruits PhysActivity Sex Smoker Stroke Veggies
   _SEGMENT_LABEL_
   %mend EM_REJECTS;
